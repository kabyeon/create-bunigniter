...
async function handleArgs(): Promise<string | null> {
	...
	for (const arg of args) {
		if (arg === "--help" || arg === "-h") {
			...
			console.log(`  ${D("Note:")}`);
			console.log(
				`    Runtime selection (Bun / Cloudflare Workers) happens via the`,
			);
			console.log(
				`    interactive wizard provided by bunigniter's bi new command.`,
			);
			console.log();
...
async function main() {
	...
	console.log();
	console.log(
		`  ${G("◇")}  ${C("create-bunigniter")} — Scaffold a Bunigniter project`,
	);
	console.log();
	...
	if (!checkBunVersion()) {
		console.log(
			`  ${R("✗")}  Bun >=1.3.0 required (current: ${process.versions.bun ?? "unknown"})`,
		);
		console.log(
			`     Install: ${C("curl -fsSL https://bun.sh/install | bash")}`,
		);
		exit(1);
	}
	...
	if (installProc.exitCode !== 0) {
		console.log(
			`  ${Y("!")}  "bun install" exited with code ${installProc.exitCode}`,
		);
	...
	console.log();
	console.log(
		`  ${G("◇")}  Launching ${useYes ? "non-interactive" : "interactive"} scaffold: ${C(biArgs.join(" "))}`,
	);
	console.log();
	...
	if (scaffoldProc.exitCode !== 0) {
		console.log(
			`  ${R("✗")}  Scaffold exited with code ${scaffoldProc.exitCode}`,
		);
